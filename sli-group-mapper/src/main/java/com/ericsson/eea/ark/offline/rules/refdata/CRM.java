...
public class CRM {
 ...
 //public Long       imsi               = null; // PK
   ...
   public CRM(CrmCustomerDeviceInfo c) {
      ...
      account_number     = c.getAccountNumber();
    //imsi               = c.getImsi();
      imsi               = getImsi(c.getImsi());
      imei               = c.getImei() == null ? null : new BigDecimal(c.getImei().toString());
      tac                = c.getTac();
      device_status      = c.getDeviceStatus();
      customer_full_name = c.getCustomerFullName();
      customer_value     = c.getCustomerValue();
      customer_type      = c.getCustomerType();
      customer_class     = c.getCustomerClass();
      customer_status    = c.getCustomerStatus();
      plan_type          = c.getPlanType();
      plan_name          = c.getPlanName();
      plan_description   = c.getPlanDescription();
      maxrate_dl         = c.getMaxrateDl();
      maxrate_ul         = c.getMaxrateUl();
      maxdata            = c.getMaxdata();
      site_data          = c.getSiteData();
      tracking_option    = c.getTrackingOption();
      notify             = c.isNotify();
   }
   ...
   public CRM(String line) {
    ...
    //if (row[n].equals("NULL")) imsi               =                0L; else imsi           = Long.parseLong      (row[n++]);
      if (row[n].equals("NULL")) imsi               =              null; else imsi           =                      row[n++] ;
   ...
   public CRM(String line, Set<String> crmColsUsed) {
    ...
    //if ((crmColsUsed == null || crmColsUsed.isEmpty()) || (!cols[ 1].equals("NULL") && (!crmColsUsed.isEmpty() && crmColsUsed.contains("imsi"              )))) imsi               = Long.parseLong  (cols[ 1]);
      if ((crmColsUsed == null || crmColsUsed.isEmpty()) || (!cols[ 1].equals("NULL") && (!crmColsUsed.isEmpty() && crmColsUsed.contains("imsi"              )))) imsi               =                  cols[ 1];
    ...
    * Default constructor.
    */
   public CRM(Map<String,Object> crmColMap) {
      ...
      Iterator<Entry<String,Object>> it = crmColMap.entrySet().iterator(); while (it.hasNext()) {

         Entry<String,Object> pairs = it.next();
         String               k     = pairs.getKey();
         Object               v     = pairs.getValue(); if (v == null) continue;

         try {

            if      (k.equalsIgnoreCase("imei"              ) && (v!=null)) imei               = v instanceof BigDecimal ? (BigDecimal) v : new BigDecimal(      (String) v);
            else if (k.equalsIgnoreCase("imsi"              )             ) imsi               = getImsi(                                                                 v);
            else if (k.equalsIgnoreCase("imei"              ) && (v!=null)) imei               = v instanceof BigDecimal ? (BigDecimal) v : new BigDecimal(      (String) v);
            else if (k.equalsIgnoreCase("tac"               ) && (v!=null)) tac                = v instanceof    Integer ?    (Integer) v : Integer.parseInt(    (String) v);
            else if (k.equalsIgnoreCase("device_status"     )             ) device_status      =                                                                 (String) v ;
            else if (k.equalsIgnoreCase("customer_full_name")             ) customer_full_name =                                                                 (String) v ;
            else if (k.equalsIgnoreCase("customer_value"    ) && (v!=null)) customer_value     = v instanceof Integer ?       (Integer) v : Integer.parseInt(    (String) v);
            else if (k.equalsIgnoreCase("customer_type"     )             ) customer_type      =                                                                 (String) v ;
            else if (k.equalsIgnoreCase("customer_class"    )             ) customer_class     =                                                                 (String) v ;
            else if (k.equalsIgnoreCase("customer_status"   )             ) customer_status    =                                                                 (String) v ;
            else if (k.equalsIgnoreCase("plan_type"         )             ) plan_type          =                                                                 (String) v ;
            else if (k.equalsIgnoreCase("plan_name"         )             ) plan_name          =                                                                 (String) v ;
            else if (k.equalsIgnoreCase("plan_description"  )             ) plan_description   =                                                                 (String) v ;
            else if (k.equalsIgnoreCase("maxrate_dl"        ) && (v!=null)) maxrate_dl         = v instanceof    Integer ?    (Integer) v : Integer.parseInt(    (String) v);
            else if (k.equalsIgnoreCase("maxrate_ul"        )             ) maxrate_ul         = v instanceof    Integer ?    (Integer) v : Integer.parseInt(    (String) v);
            else if (k.equalsIgnoreCase("site_data"         )             ) site_data          =                                                                 (String) v ;
            else if (k.equalsIgnoreCase("tracking_option"   )             ) tracking_option    =                                                                 (String) v ;
            else if (k.equalsIgnoreCase("notify"            ) && (v!=null)) notify             = v instanceof    Boolean ?    (Boolean) v : Boolean.parseBoolean((String) v);
         }
         catch (NumberFormatException ex) {

            log.error("Error processing '"+k+"': ", ex); throw ex;
         }
      }
   }


   private String getImsi(Object imsi) {

      String res = null; try {

//         res = (imsi instanceof Long) ? Util.encImsi((Long)imsi) : (String) imsi;
          if ( ! Util.getDefaultEncType().longToLong &&
                  imsi instanceof Long) {
              res = Util.encImsi((Long)imsi);
          } else {
              res = imsi == null ? null : imsi.toString();
          }
      }
      catch (Exception ex) {
...
//   }
   ...
   public Object getSite_dataByKey(String key, String dataType) {
      ...
      if (isSite_data()) {
         ...
         try {
            ...
            log.warn("error parsing site_data trying to get key: " + key + e.getMessage());
            log.debug("exception: ", e);
            return null;
         }
         catch (NumberFormatException e) {
            log.warn("error NumberFormatException site_data trying to parse key value: " + key + e.getMessage());
            log.debug("exception: ", e);
            return null;
         }
   ...
   public static synchronized void query(HashMap<Long,CRM> crmhash
      ...
      try {
         ...
         String query = "select " + selectCols + " from " + RuleEngine.config.databaseSchema() + "." + RuleEngine.config.CRMtablename();
...
//         log.info("query: " + query);
         ...
         ResultSet resultset = sql.executeQuery(query); while (resultset.next()) {
          ...
          //if ((crmUsedCols == null || crmUsedCols.isEmpty()) || ((!crmUsedCols.isEmpty() && crmUsedCols.contains("imsi"              )))) c.setImsi              (resultset.getBigDecimal("imsi"              ).longValue());
            if ((crmUsedCols == null || crmUsedCols.isEmpty()) || ((!crmUsedCols.isEmpty() && crmUsedCols.contains("imsi"              )))) c.setImsi              (resultset.getString    ("imsi"              ));
          ...
          //crmhash.put(c.imsi, c);
            crmhash.put((long) c.imsi.hashCode(), c);
         }
         resultset.close();
         ...
         log.error("Exception loading from table" + RuleEngine.config.CRMtablename()+"'", ex);
   ...
   public static synchronized void load(HashMap<Long, CRM> crmhash, Set<String> crmColsUsed) throws IOException {
      ...
      BufferedReader reader = null; try {
         ...
         fr     = new FileReader(RuleEngine.config.CRMfilename());
