...
//
//         int chars_read = data_length - j; if (chars_read > 16) chars_read = 16;
//
//         buffer.append(dump(display_offset)).append("  ");
//         for (int k = 0; k < 16; k++) {
//
//            if (k < chars_read) buffer.append(dump(data[ k + j ]));
//            else                buffer.append("  ");
//
//            buffer.append(' '); if (k == 7) buffer.append(' ');
//         }
//
//         buffer.append(" |");
//         for (int k = 0; k < chars_read; k++) {
//
//            if ((data[ k + j ] >= ' ') && (data[ k + j ] < 127)) buffer.append(( char ) data[ k + j ]);
//            else                                                 buffer.append('.');
//         }
//         buffer.append('|');
//
...
//   }
   ...
   public static void dump(final byte[]       data
      ...
      String text = dump(data, offset, index, length);
    ...
    * @param index   initial index into the byte array
    *
    * @exception IOException                     is thrown if anything goes wrong writing the data to stream
    * @exception ArrayIndexOutOfBoundsException  if the index is outside the data array's bounds
    * @exception IllegalArgumentException        if the output stream is null
    */
   public synchronized static void dump(final byte[]       data
      ,                                 final long         offset
      ,                                 final OutputStream stream
   ...
   public static String dump(final byte[] data
      ...
      return dump(data, offset, index, data.length);
   }
...
//
//         int chars_read = data.length - j; if (chars_read > 16) chars_read = 16;
//
//         buffer.append(dump(display_offset)).append("  ");
//         for (int k = 0; k < 16; k++) {
//
...
//   }
   ...
   private static String dump(final byte value) {
      ...
      StringBuffer buf = new StringBuffer(); {
         ...
         buf.setLength(0); for (int j = 0; j < 2; j++) {
   ...
   public static String toHex(final byte[] value) {
      ...
      StringBuffer retVal = new StringBuffer(); {
         ...
         retVal.append('['); for(int x = 0; x < value.length; x++) {

            if (x>0) retVal.append(", "); retVal.append(toHex(value[x]));
         }
         retVal.append(']');
      }
   ...
   public static String toHex(final short[] value) {
      ...
      StringBuffer retVal = new StringBuffer(); {
         ...
         retVal.append('['); for(int x = 0; x < value.length; x++) {
            ...
            retVal.append(toHex(value[x])).append(", ");
         }
         retVal.append(']');
      }
   ...
   public static String toHex(final byte[] value, final int bytesPerLine) {
        ...
        final StringBuffer formatString = new StringBuffer(); {
           ...
           for (int i = 0; i < digits; i++) formatString.append('0');
           formatString.append(": ");
        }
        ...
        int i = -1; for(int x = 0; x < value.length; x++) {
   ...
   public static void dump( InputStream in, PrintStream out, int start, int bytesToDump ) throws IOException {
      ...
      if (bytesToDump == -1) {
         ...
         int c = in.read(); while (c != -1) {

            buf.write(c); c = in.read();
         }
      }
      else {

         int bytesRemaining = bytesToDump; while (bytesRemaining-- > 0) {
   ...
   /**
    ...
    private static char[] toHexChars(long pValue, int nBytes) {
       ...
       do {
          ...
          result[--charPos] = _hexcodes[(int) (value & 0x0F)]; value >>>= 4;
       } while (charPos > 1);
   ...
   public static void main(String[] args) throws Exception {
      ...
      File        file =                         args.length == 0 ? null                  : new File(args[0]);
      InputStream in   = new BufferedInputStream(args.length == 0 ? System.in             : new FileInputStream(file));
      byte[]      b    = new byte[               args.length == 0 ? System.in.available() : (int)file.length()]; in.read(b);

      System.out.println(Hexdump.dump(b, 0, 0)); in.close();
   }
}
