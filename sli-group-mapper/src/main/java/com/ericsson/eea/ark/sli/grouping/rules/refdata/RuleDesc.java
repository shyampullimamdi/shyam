...
//import com.ericsson.eea.sli.grouping.rules.VerifyRules;
//import com.ericsson.eea.sli.grouping.rules.utils.RuleMetaData;
...
public class RuleDesc {
   ...
   public RuleDesc(String ruleDesc_location_row) {
      ...
      if (row.length != 10) {
         ...
         log.error("invalid ruleDesc line ignored"); return;
      }
      ...
      /**/                        this.message_soc    = "\\N";
      /**/                        this.message_cc     = "\\N";
      /**/                        this.message_cclite = "\\N";
      /**/                        this.kpi_name       = "\\N";
      /**/                        this.kpi_source     = "\\N";
      if (!"NULL".equals(row[3])) this.impact         = row[3];
      if (!"NULL".equals(row[4])) this.reason         = row[4];
      if (!"NULL".equals(row[5])) this.message_soc    = row[5];
      if (!"NULL".equals(row[6])) this.message_cc     = row[6];
      if (!"NULL".equals(row[7])) this.message_cclite = row[7];
      if (!"NULL".equals(row[8])) this.kpi_name       = row[8];
      if (!"NULL".equals(row[9])) this.kpi_source     = row[8];
   }
   ...
   public static void load(Map<String,RuleDesc> ruleDeschash
      ,                    SimpleConfig         config) throws IOException {

      FileReader     fr     = null;
      BufferedReader reader = null; try {

         fr     = new FileReader(config.ruleDescfilename());
   ...
   public static void query(Map<String, RuleDesc> ruleDeschash, SimpleConfig config, SQLConnection reSqlConnection) {
      ...
      try {
         ...
         while (resultset.next()) {
            ...
            String impact         = resultset.getString("impact");         if (resultset.wasNull()) impact         = "\\N";
            String reason         = resultset.getString("reason");         if (resultset.wasNull()) reason         = "\\N";
            String message_soc    = resultset.getString("message_soc");    if (resultset.wasNull()) message_soc    = "\\N";
            String message_cc     = resultset.getString("message_cc");     if (resultset.wasNull()) message_cc     = "\\N";
            String message_cclite = resultset.getString("message_cclite"); if (resultset.wasNull()) message_cclite = "\\N";
            String kpi_name       = resultset.getString("kpi_name");       if (resultset.wasNull()) kpi_name       = "\\N";
            String kpi_source     = resultset.getString("kpi_source");     if (resultset.wasNull()) kpi_source     = "\\N";
